...
def update_answer_model():
    ...
    st.session_state.agent_executer = llm_controller.create_agent_executer(
        ollama_model_name, 
        app_settings.GENERATION_TEMPERATURE
    )

    st.session_state.final_chain = RunnableWithMessageHistory(
        st.session_state.agent_executer,
        get_session_history,
...
def chat_with_llm(session_id, input):
    for chunk in st.session_state.final_chain.stream({'input': input}, config={'configurable': {'session_id': session_id}}):
        if "output" in chunk:
            yield chunk["output"]
...
if user_query := st.chat_input("What is in your mind?"):
    if "retriever" not in st.session_state:
        st.error("Retrieverwas not set.")

    elif "agent_executer" not in st.session_state:
        st.error("Agent Executer was not set.")
